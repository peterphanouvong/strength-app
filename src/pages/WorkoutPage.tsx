  ...
  return (
    <div className="min-h-screen pb-28">
      ...
      <main className="max-w-xl mx-auto px-5 py-5">
        ...
        >
          <Stat label="Duration" value={live ? formatElapsed(elapsed) : '—'} accent="text-primary" />
          ...
          >
            <div className="max-w-xl mx-auto bg-surface-deep border border-white/15 rounded-2xl px-4 py-3 shadow-xl">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-[0.6875rem] font-medium text-secondary truncate">Rest · {rest.label}</p>
          ...
          {SET_TYPES.map((option) => {
            ...
            return (
              ...
              >
                <span
                  className={cn(
                    'w-10 h-10 rounded-xl bg-surface-deep/60 flex items-center justify-center text-lg font-bold tabular-nums flex-shrink-0',
                    option.color
                  )}
                >
                  {option.letter}
                ...
                <span className="flex-1 min-w-0">
                  ...
                  <span className="block text-xs text-secondary mt-0.5">{option.hint}</span>
                </span>
                {selected && <Check className="w-5 h-5 text-primary flex-shrink-0" strokeWidth={3} />}
              </button>
            );
          })}
        </div>
      ...
      >
        ...
        <p className="text-xs text-secondary text-center leading-relaxed mt-4 px-4">
      ...
      <BottomSheet open={cancelOpen} onClose={closeCancelSheet} title="End workout">
        ...
        {confirmDiscard ? (
          <div className="mt-2 bg-danger/10 rounded-xl px-4 py-3.5">
